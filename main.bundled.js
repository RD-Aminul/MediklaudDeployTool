...
var require_patcher = __commonJS({
  "src/patcher.js"(exports2, module2) {
    var fs2 = require("fs");
    ...
    function applyPatchRules2(rules, values, onLine) {
...
`);
        ...
        if (!fs2.existsSync(rule.file)) {
          throw new Error(`Patch target not found: ${rule.file}`);
        }
        let content = fs2.readFileSync(rule.file, "utf8");
        ...
        fs2.writeFileSync(rule.file, content, "utf8");
...
var require_runner = __commonJS({
  "src/runner.js"(exports2, module2) {
    var { spawn, exec, execFileSync } = require("child_process");
    var path2 = require("path");
    var fs2 = require("fs");
    ...
    function getCurrentBranch2(repoPath) {
      try {
...
`);
    ...
    function dotnetPublish2(apiRepoPath, csprojRelative, outputDir, runtimeIdentifier, onLine, onProgress) {
      const csprojPath = path2.join(apiRepoPath, csprojRelative);
    ...
    function shutdownDotnetBuildServers2(repoPath, onLine) {
...
`);
    ...
    var APP_OFFLINE2 = "app_offline.htm";
    function takeAppOffline2(appDir, onLine) {
      fs2.mkdirSync(appDir, { recursive: true });
      fs2.writeFileSync(
        path2.join(appDir, APP_OFFLINE2),
        "<html><body><h1>Deploying\u2026</h1></body></html>",
        "utf8"
      );
      onLine(`
Placed ${APP_OFFLINE2} in ${appDir} (asking IIS to release the app)
`);
    }
    function bringAppOnline2(appDir, onLine) {
      const p = path2.join(appDir, APP_OFFLINE2);
      if (fs2.existsSync(p)) {
        fs2.rmSync(p, { force: true });
        onLine(`
Removed ${APP_OFFLINE2} \u2014 app can start again
`);
      }
    }
    async function clearDirContents2(dir, forbiddenPaths, onLine, keepNames) {
      ...
      for (const forbidden of forbiddenPaths || []) {
        ...
        const f = path2.resolve(forbidden);
        if (resolved === f || resolved.startsWith(f + path2.sep) || f.startsWith(resolved + path2.sep)) {
...
`);
      ...
      const entries = fs2.readdirSync(resolved).filter((e) => !keep.has(e.toLowerCase()));
      let removed = 0;
      for (const entry of entries) {
        const target = path2.join(resolved, entry);
        let lastErr = null;
...
`);
        ...
        if (lastErr) {
          throw new Error(
            `Could not delete "${target}" \u2014 it is still locked by another process (usually IIS/w3wp serving this folder, or a running dotnet process). Stop the site/app pool using this folder and run again. [${lastErr.code}]`
          );
        }
        removed++;
      }
...
`);
    ...
    function sevenZipArchive2(sevenZipExe, workingDir, archiveName, folders, onLine, onProgress) {
      if (!fs2.existsSync(sevenZipExe)) {
        return Promise.reject(new Error(`7-Zip not found at "${sevenZipExe}". Fix paths.sevenZipExe in Settings.`));
      }
      const archivePath = path2.join(workingDir, archiveName);
...
`);
    ...
    function zipFolder2(sourceDir, zipFilePath, onLine, onProgress) {
      return new Promise((resolve, reject) => {
        fs2.mkdirSync(path2.dirname(zipFilePath), { recursive: true });
...
`));
    ...
    module2.exports = {
      runCommand,
      killActiveProcess: killActiveProcess2,
      getCurrentBranch: getCurrentBranch2,
      gitPull: gitPull2,
      yarnBuild: yarnBuild2,
      dotnetPublish: dotnetPublish2,
      shutdownDotnetBuildServers: shutdownDotnetBuildServers2,
      clearDirContents: clearDirContents2,
      takeAppOffline: takeAppOffline2,
      bringAppOnline: bringAppOnline2,
      APP_OFFLINE: APP_OFFLINE2,
      sevenZipArchive: sevenZipArchive2,
...
// src/iis.js
var require_iis = __commonJS({
  "src/iis.js"(exports2, module2) {
        ...
        execFile(appcmdExe, args, { windowsHide: true }, (err, stdout, stderr) => {
          ...
          if (err) {
            if (/insufficient permissions|redirection\.config/i.test(output)) {
              const e2 = new Error("IIS control needs administrator rights \u2014 run this app as Administrator.");
              e2.adminRequired = true;
              reject(e2);
              return;
            }
    ...
    async function stopIis2(appcmdExe, siteName, appPoolName, onLine) {
      if (!fs2.existsSync(appcmdExe)) {
...
`);
    ...
    async function startIis2(appcmdExe, siteName, appPoolName, stopped, onLine) {
...
`);
    ...
    module2.exports = { stopIis: stopIis2, startIis: startIis2, runAppcmd };
  }
});

// src/pathDetector.js
var require_pathDetector = __commonJS({
  "src/pathDetector.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    ...
    function listDrives() {
      ...
      for (let c = 65; c <= 90; c++) {
        ...
        try {
          if (fs2.existsSync(root)) drives.push(root);
        } catch {
        }
      }
      return drives;
    }
    function findFolders2(targetNames, onLine, options = {}) {
...
`);
    ...
    module2.exports = { findFolders: findFolders2, listDrives };
  }
});
